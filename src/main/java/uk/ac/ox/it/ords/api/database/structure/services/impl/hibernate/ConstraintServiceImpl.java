...
public class ConstraintServiceImpl extends StructureServiceImpl
	...
	public MessageEntity getConstraint(int dbId, String instance,
		...
		if (!this.checkConstraintExists(tableName, constraintName, databaseName, userName, password)){
			throw new NotFoundException();
		}
	...
	public void createConstraint(int dbId, String instance, String tableName,
			String constraintName, ConstraintRequest newConstraint,  boolean staging) throws Exception {
        String query;
        ...
        if (!this.checkTableExists(tableName, databaseName, userName, password)) {
            log.error("Tried to create constraint %s for non-existant table %s", 
                        constraintName, 
                        tableName);
            //message = String.format(emd.getMessage("Rst052").getText(), tableName);
            throw new NotFoundException();
        }
        ...
        Object result = this.singleResultQuery(conIdQuery, databaseName, userName, password);
        if ( result != null ){
            // Actually generate a name for the constraint
            int conId = ((Number)result).intValue();
        ...
        if (this.checkConstraintExists(tableName, uniqueConstraintName, databaseName, userName, password)) {
            log.error("Tried to create duplicate constraint name %s on table %s", 
